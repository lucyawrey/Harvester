...
function __scribble_class_model(_element, _model_cache_name) constructor {
	...
	static __get_glyph_data = function(_index, _page) {
		...
		if (!__allow_glyph_data_getter) {
			__scribble_error(
				"Getting glyph data requires either:\n- Call `.allow_glyph_data_getter()` on the element\n- Set `SCRIBBLE_FORCE_GLYPH_DATA_GETTER` to `true`"
			);
		}
