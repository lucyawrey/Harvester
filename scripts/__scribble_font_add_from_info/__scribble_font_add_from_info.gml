	...
	try {
		...
		repeat (array_length(_info_glyph_names)) {
			...
			if (is_struct(_struct)) {
				...
				__scribble_trace(
					$"Warning! Failed to access glyph data for char \"{
						_glyph
					}\" in font \"{_name}\""
				);
			}
		...
		if (
			(_texture_group != undefined)
			&& (!__scribble_texture_group_get_ready(_texture_group))
		) {
		...
		if (_ascender <= 0) {
			...
			__scribble_trace(
				"Warning! Font \"",
				_name,
				"\" has an invalid ascender (value is less than or equal to 0). Estimated a value of ",
				_ascender
			);
		}
