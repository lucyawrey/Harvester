...
function __scribble_class_element(_string, _unique_id) constructor {
	...
	static align = function(_halign = __starting_halign, _valign = __starting_valign) {
		...
		if (_valign == "pin_top") {
			_valign = __SCRIBBLE_PIN_TOP;
		}
		if (_valign == "pin_middle") {
			_valign = __SCRIBBLE_PIN_MIDDLE;
		}
		if (_valign == "pin_bottom") {
			_valign = __SCRIBBLE_PIN_BOTTOM;
		}
	...
	static blend = function(_colour, _alpha) {
		...
		if (_alpha != undefined) {
			__blend_alpha = clamp(_alpha, 0, 1);
		}
	...
	static __get_model = function(_allow_create) {
		...
		if (__flushed || (__text == "")) {
			...
			if (__model_cache_name_dirty) {
				...
				buffer_write(_buffer, buffer_u8, 0x3A);
				buffer_write(_buffer, buffer_text, string(__allow_text_getter));
				buffer_write(_buffer, buffer_u8, 0x3A);
				buffer_write(_buffer, buffer_text, string(__allow_glyph_data_getter));
				buffer_write(_buffer, buffer_u8, 0x3A);
				buffer_write(_buffer, buffer_text, string(__allow_line_data_getter));
				buffer_write(_buffer, buffer_u8, 0x3A);
